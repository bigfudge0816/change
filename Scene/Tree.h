...
#define INITIAL_NUM_ITERATIONS 1
...
#define MAXIMUM_BRANCH_RADIUS 1.0f

#define INITIAL_NUM_ATTR_PTS 100000
...
public:
	...
	void IterateGrowth(KdTree& currentKdTree, std::vector<AttractorPoint> &attractorPoints, glm::vec3& minAttrPt, glm::vec3& maxAttrPt, TreeParameters& treeParams, bool useGPU = false);
    void PerformSpaceColonization(KdTree& currentKdTree,std::vector<AttractorPoint>& attractorPoints, glm::vec3& minAttrPt, glm::vec3& maxAttrPt, TreeParameters& treeParameters, bool useGPU);
    void PerformSpaceColonizationCPU(KdTree& currentKdTree, TreeParameters& treeParameters);
    void PerformSpaceColonizationGPU(KdTree& currentKdTree,std::vector<AttractorPoint>& attractorPoints, glm::vec3& minAttrPt, glm::vec3& maxAttrPt, bool& reconstructUniformGrid, bool& resetAttrPtState);
    void RemoveAttractorPoints(KdTree&currentKdTree);

    float ComputeQAccumRecursive(TreeParameters& treeParams,TreeBranch& branch);
    void ComputeBHModelBasipetalPass(TreeParameters& treeParams);
    void ComputeResourceFlowRecursive(TreeParameters& treeParams,TreeBranch& branch, float resource);
    void ComputeBHModelAcropetalPass(TreeParameters& treeParams);

    void AppendNewShoots(int n,  TreeParameters& treeParams);

    float ComputeBranchRadiiRecursive(TreeBranch& branch,  TreeParameters& treeParams);
    void ComputeBranchRadii( TreeParameters& treeParams);

	void ResetState(bool useGPU);// Reset the state of each bud in the tree during the iterative algorithm
